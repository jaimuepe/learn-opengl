...
int main() {
  ...
  glNamedBufferData(groundVBO, sizeof(groundVertices), groundVertices,
                    GL_STATIC_DRAW);
  ...
  glVertexArrayVertexBuffer(groundVAO, 0, groundVBO, 0, 5 * sizeof(float));
  glVertexArrayAttribBinding(groundVAO, 0, 0);

  // texcoords
  glEnableVertexArrayAttrib(groundVAO, 1);
  glVertexArrayAttribFormat(groundVAO, 1, 2, GL_FLOAT, GL_FALSE, 0);
  glVertexArrayVertexBuffer(groundVAO, 1, groundVBO, 3 * sizeof(float),
                            5 * sizeof(float));
  glVertexArrayAttribBinding(groundVAO, 1, 1);
...
gpu::Shader shader("shader.vs", "shader.fs");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      model =
          glm::translate(model, glm::vec3{0.0f, 2.0f, 0.0f} + cubePositions[i]);
...
void process_input(GLFWwindow *window) {
  ...
  if (front != 0 || right != 0) {
    ...
    glm::vec3 dirWorldSpaceXZ = glm::cross(up, glm::cross(dirWorldSpace, up));
    dirWorldSpaceXZ = glm::normalize(dirWorldSpaceXZ);

    camera.translate(dirWorldSpaceXZ * speed);
  }
}
