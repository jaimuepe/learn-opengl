...
int main() {
  ...
  glVertexArrayVertexBuffer(lightVAO, 0, VBO, 0, 6 * sizeof(float));
...
gpu::Shader lightCubeShader("light-cube.vs", "light-cube.fs");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glm::vec3 lightPosViewSpace = view * glm::vec4{lightPos, 1.0f};

    lightingShader.use();

    lightingShader.setVec3("objectColor", 1.0f, 0.5f, 0.31f);

    lightingShader.setVec3("lightPos", glm::vec3{lightPosViewSpace});
    ...
    lightingShader.setMat4("invViewModel", glm::inverse(view * model));
