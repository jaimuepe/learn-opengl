...
gpu::Shader lightingShader("lighting-maps.vs", "lighting-maps.fs");
gpu::Shader lightCubeShader("light-cube.vs", "light-cube.fs");
  ...
  GLuint containerEmissionMap;
  glCreateTextures(GL_TEXTURE_2D, 1, &containerEmissionMap);

  {
    ...
    std::string texPath = getTexturePath("matrix.jpg");

    unsigned char *data =
        stbi_load(texPath.c_str(), &texWidth, &texHeight, &texNrChannels, 0);

    glTextureParameteri(containerEmissionMap, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTextureParameteri(containerEmissionMap, GL_TEXTURE_WRAP_T, GL_REPEAT);

    glTextureParameteri(containerEmissionMap, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTextureParameteri(containerEmissionMap, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    glTextureStorage2D(containerEmissionMap, 1, GL_RGB8, texWidth, texHeight);
    glTextureSubImage2D(containerEmissionMap, 0, 0, 0, texWidth, texHeight,
                        GL_RGB, GL_UNSIGNED_BYTE, data);

    glGenerateTextureMipmap(containerEmissionMap);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glBindTextureUnit(2, containerEmissionMap);
